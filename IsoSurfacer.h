...
class VTK_EXPORT IsoSurfacer : public vtkAlgorithm {
  ...
  protected:
    ...
    //QUESTION 8
      ...
      vector<double> vectorp1p2(3); //Vecteur
      double rapport;

	  //Get value of edge ends
        double value1 = Input->GetPointData()->GetScalars()->GetComponent(edge.first,0);
        double value2 = Input->GetPointData()->GetScalars()->GetComponent(edge.second,0);
	  //Get coordinates of edge ends
        ...
        rapport=(Value-min(value1,value2))/(abs(value2-value1));

        if(value1<value2)
        {
            vectorp1p2[0]=p2[0]-p1[0];
			vectorp1p2[1] = p2[1] - p1[1];
			vectorp1p2[2] = p2[2] - p1[2];

            p[0]=p1[0]+rapport*vectorp1p2[0];
			p[1] = p1[1] + rapport*vectorp1p2[1];
			p[2] = p1[2] + rapport*vectorp1p2[2];
        }
        else
        {
			vectorp1p2[0] = p1[0] - p2[0];
			vectorp1p2[1] = p1[1] - p2[1];
			vectorp1p2[2] = p1[2] - p2[2];

			p[0] = p2[0] + rapport*vectorp1p2[0];
			p[1] = p2[1] + rapport*vectorp1p2[1];
			p[2] = p2[2] + rapport*vectorp1p2[2];
        }

		vector<double> sortie;
		sortie.push_back(p[0]);
		sortie.push_back(p[1]);
		sortie.push_back(p[2]);
